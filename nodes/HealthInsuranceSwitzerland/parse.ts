...
export default async function parse(rawInputs: any): Promise<IInput> {
  ...
  const version = rawInputs?.version ?? "";
  let franchise = rawInputs?.franchise ?? "1000";

  // We format them as needed in the sheet
  const age: any = rawInputs?.age ?? (settings as any)?.ageSelections[language][0];
  ...
  const postal = rawInputs?.postalCode ?? (settings as any)?.defaultPostalCode[language];
  ...
  const spreadsheetId = rawInputs.spreadsheetId ?? "1mHOPog6kosRTqRwkCjOiY1xGrcr_QLZRTdFLh1a4Xmo";
